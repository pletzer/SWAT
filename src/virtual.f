...
!! sum HRU results for subbasin 
     ...
     &          + (hhsedy(j,ii) + ubntss(ii)) * hru_fr(j)
      ...
      !! subbasin averages: sediment
        ...
        surqno3(j) = max(1.e-12,surqno3(j))
        latno3(j) = max(1.e-12,latno3(j))
        no3gw(j) = max(1.e-12,no3gw(j))
        surqsolp(j) = max(1.e-12,surqsolp(j))
        minpgw(j) = max(1.e-12,minpgw(j))
        sedorgn(j) = max(1.e-12,sedorgn(j))
        sedorgp(j) = max(1.e-12,sedorgp(j))
        sedminpa(j) = max(1.e-12,sedminpa(j))
        sedminps(j) = max(1.e-12,sedminps(j))
